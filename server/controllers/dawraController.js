...
import DawraRegistration from '../models/DawraRegistration.js';
import nodemailer from 'nodemailer';

// Register for Dawra
const registerDawra = asyncHandler(async (req, res) => {
  const {
    name, birthdate, state, school, phone, email, telegram,
    memorization, narration, tajweed, sessionTime, notes, commitment
  } = req.body;
  if (!name || !birthdate || !state || !school || !phone || !email || !memorization || !sessionTime || commitment === undefined) {
    res.status(400);
    throw new Error('جميع الحقول المطلوبة يجب ملؤها');
  }
  const registration = await DawraRegistration.create({
    name, birthdate, state, school, phone, email, telegram,
    memorization, narration, tajweed, sessionTime, notes, commitment
  });
  res.status(201).json({ message: 'تم التسجيل في الدورة بنجاح', registration });
});

// Get All Dawra Registrations (Admin only)
const getDawraRegistrations = asyncHandler(async (req, res) => {
  const registrations = await DawraRegistration.find().sort({ createdAt: -1 });
...
const deleteRegistration = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const registration = await DawraRegistration.findById(id);
...
// Send Email to Dawra Registrants (Admin only)
const sendDawraEmail = asyncHandler(async (req, res) => {
...
export { registerDawra, getDawraRegistrations, deleteRegistration, sendDawraEmail };